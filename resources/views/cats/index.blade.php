    ...
    <section class="mt-12">
        ...
        @else
            <div class="mb-6 flex flex-wrap items-center justify-between gap-3">
                <p class="subtle-text">{{ $cats->total() }} profil(s) visible(s) actuellement.</p>
                <a href="{{ route('contact') }}" class="btn-ghost px-0">Poser une question avant d'adopter</a>
            </div>
                    ...
                    <a href="{{ route('cats.show', $cat) }}" class="cat-card cat-card-link group flex h-full flex-col" aria-label="Voir la fiche de {{ $cat->name }}">
                        ...
                        <div class="flex flex-1 flex-col p-6">
                            <div class="flex items-start justify-between gap-4">
                                <div class="min-w-0">
                                    <h2 class="text-3xl leading-tight font-semibold text-amber-950 transition group-hover:text-amber-800 sm:text-[2.2rem]">
                                        {{ $cat->name }}
                                    </h2>
        ...
        <div class="luminous-panel p-8 sm:p-10">
            <div class="grid gap-8 lg:grid-cols-[1fr_auto] lg:items-center">
                ...
                <div class="flex flex-wrap gap-3 lg:justify-end">
                    <a href="{{ route('about') }}" class="btn-secondary border-white/15 bg-white text-amber-900">Notre approche</a>
